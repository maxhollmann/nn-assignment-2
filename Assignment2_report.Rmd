...
When wondering on how to solve the machine learning problem, we tried to come up with a model that somewhat mimicks the cognitive process we are undergoing when identifying a guest from a non-guest. We noticed that, instead of scanning the whole document, we would usually quickly scan for a name, consider only the couple of words surrounding a name, and derive from its syntax what role the person will play in the podcast episode. This led us to chunk our data into portions with $n$ words to the left and the right, thus of the structure:
...
Additionally, we experimented with batch size, where we agreed to use segments of 256 inputs, and optimizers, with *Stochastic Gradient Descent (SGD)* performing the best. The final network we ended up with achieved a validation accuracy of 75.31% and had the following architecture:
...
A more sophisticated approach for word vectors is to construct them from one's own dataset, as they will be trained on the relevant domain (i.e., how words are related to each other in podcast episode descriptions). Keras allows for such constructions with the `Embedding` layer


- make embedding / word vectors ourselves with most frequent 4000 or k words (p = 32)
- works better because trained in this domain
- Recurrent network: LSTM neurons to understand syntax 



## III. Convolutional LSTM
...
- amount of times name is mentioned in dataset source (int; also unlabelled cases)
...
We applied standard supervised machine learning techniques to the classification problem and found a couple of them to work out relatively well. The best ones were *Learning Vector Quantization (LVQ)*, *Support Vector Machines (SVM)*, *Gradient Boosting Machines (GBM)*, and *Generalized Linear Modelling (GLM)*. GBM and GLM achieved pretty high results, with GLM slightly outperforming GBM with a mean cross-validated accuracy of 76.14%. See the graph below for a summary of the accuracies:
...
Premature models often ended up classifying only to one class ("no guest"), just because of non-guests making up a larger portion of the labelled outcomes. Thus, a lower bound for our accuracies was the prior probability P("no guest") = 63.8%.
...
- question on how to create best architecture
- followed theory, but might be different for our problem
